    ...
    def Execute(self, commandData):
        ...
        try:
            ...
            try:
                ...
                viewSchedule = self.parent.get_view_by_name_to_element('ZH_Ведомость изменений{}'.format(subText))


                if viewSchedule is None:
                    viewSchedule = createKeySchedule(subText)
                        ...
                        for enum, numRev in enumerate(sortedRevSheetDiscript.keys(), 0):
                            for numSheet in sortedRevSheetDiscript[numRev].keys():
                                for discription, reason in sortedRevSheetDiscript[numRev][numSheet]:
                                    ...
                                    try:
                                        elements_to_add.LookupParameter('Изм._Изм.{}'.format(subText)).Set(setNumRev)
                                        elements_to_add.LookupParameter('Изм._Лист{}'.format(subText)).Set(setNumSheet)
                                        elements_to_add.LookupParameter('Изм._Содержание изменений{}'.format(subText)).Set(discription)
                                        elements_to_add.LookupParameter('Изм._Код{}'.format(subText)).Set(reason)
                                    except Exception as e:
                                        self.parent.showLineError(e)
                except Exception as e:
                    t.Commit()
