            ...
            try:
                ...
                if len(revision_clouds) != 0:
                    for revCloud in revision_clouds:
                        num_revCloud = (
                            revCloud.get_Parameter(BuiltInParameter.REVISION_CLOUD_REVISION_NUM).AsString()).rstrip('‪')
                ...
                for revCloud in revision_clouds:
                    ...
                    if numberRev not in resultToShedule:
                        resultToShedule[numberRev] = {numberSheet: [descriptionRev]}
                    else:
                        if numberSheet not in resultToShedule[numberRev]:
                            resultToShedule[numberRev][numberSheet] = [descriptionRev]
                        else:
                            resultToShedule[numberRev][numberSheet].append(descriptionRev)
                    ...
                    try:
                        ...
                        for enum, numRev in enumerate(sortedRevSheetDiscript.keys(), 0):
                            for numSheet in sortedRevSheetDiscript[numRev].keys():
                                for discription in sortedRevSheetDiscript[numRev][numSheet]:
...
class RevisionForm(Form):
    ...
    def middlePanel(self):
        ...
        self.textBoxAlbomName = TextBox(Text = 'Без альбома',
                                         Size = Size(230, 30),
                                        Location = Point(10, 45),
                                        TextAlign = HorizontalAlignment.Center)
        # for chapter in ['Без альбома','АР', 'КР', 'АС0.1', 'АС1.1']:
        #     self.textBoxAlbomName.Items.Add(chapter)
