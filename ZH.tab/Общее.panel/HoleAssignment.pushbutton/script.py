...
#     MessageBox.Show('На ремонте)', 'Предупреждение')
#     sys.exit()
...
class SettingForm(Form):
    ...
    def mainPanel(self):
        try:
            ...
            """-------------Чекбоксы для задания позиций отверстиям-----------------------"""

            sep = Panel(Name='SideBorders',
                           Height=1,
                            Width = self.Width - 20,
                           Location=Point(10, 195))
            self.middlePanel.Controls.Add(sep)


            self.size3DViewLabel = Label(Text='Проставить позицию в:',
                                         Location=Point(10, 200),
                                         Size=Size(230, 30))
            self.middlePanel.Controls.Add(self.size3DViewLabel)


            self.cb_PlaneSquare = CheckBox(Text='Прямоугольное на плоскости',
                                         Location=Point(10, 225),
                                         Size=Size(230, 30),
                                           Checked = self.jsonSettings['OnPlaneSquare'])
            self.middlePanel.Controls.Add(self.cb_PlaneSquare)

            self.cb_LevelSquare = CheckBox(Text='Прямоугольное на уровне',
                                           Location=Point(10, 255),
                                           Size=Size(230, 30),
                                           Checked = self.jsonSettings['OnLevelSquare'])
            self.middlePanel.Controls.Add(self.cb_LevelSquare)

            self.cb_PlaneCircle = CheckBox(Text='Круглое на плоскости',
                                           Location=Point(10, 285),
                                           Size=Size(230, 30),
                                           Checked = self.jsonSettings['OnPlaneCircle'])
            self.middlePanel.Controls.Add(self.cb_PlaneCircle)

            self.cb_LevelCircle = CheckBox(Text='Круглое на уровне',
                                           Location=Point(10, 315),
                                           Size=Size(230, 30),
                                           Checked = self.jsonSettings['OnLevelCircle'])
            self.middlePanel.Controls.Add(self.cb_LevelCircle)
            """---------------------------------------------------"""
        except Exception as e:
            import sys
    ...
    def saveSettings(self, template = False):
        ...
        if template == False:
            ...
            self.jsonSettings['OnPlaneSquare'] = self.cb_PlaneSquare.Checked
            self.jsonSettings['OnLevelSquare'] = self.cb_LevelSquare.Checked
            self.jsonSettings['OnPlaneCircle'] = self.cb_PlaneCircle.Checked
            self.jsonSettings['OnLevelCircle'] = self.cb_LevelCircle.Checked
        else:
            pass
...
class CopyElementsHandler(IExternalEventHandler):
    ...
    def Execute(self, commandData):
            ...
            try:
                ...
                for linked_instance, link_doc in linked_documents:
                    ...
                    if element:
                        ...
                        if cat and cat.BuiltInCategory == BuiltInCategory.OST_GenericModel:
                            return element, linked_instance
                return None
            except Exception as e:
                showLineError(e)

        def showLineError(error):
            import traceback
            import sys
            print('-------------- ERROR ---------------------')
        ...
        def get_type_by_name(family_name, type_name):
            from Autodesk.Revit.DB import FilteredElementCollector

            types = FilteredElementCollector(self.doc).WhereElementIsElementType().ToElements()
            for t in types:
                if t.FamilyName == family_name and t.get_Parameter(BuiltInParameter.SYMBOL_NAME_PARAM).AsValueString() == type_name:
                    return t
            return None

        paramToMiss = ['Тип', 'Семейство и типоразмер', 'Семейство и типоразмер', 'Семейство', 'Рабочий набор', 'Отметка от уровня', 'Смещение от главной модели']
        def get_all_instance_parameters(linked_Element):
            ...
            data = {}
            for p in linked_Element.Parameters:
                if p:
                    pName = p.Definition.Name
                    if pName not in paramToMiss:
                            data.setdefault(p.Definition.Name, {"STORAGE_TYPE": p.StorageType, "VALUE": get_value(p)})
            return data
        ...
        try:
            ...
            if self.elements != None:
                for linkedElementId in self.elements:
                    ...
                    tempLinkedDocData = copyFind_element_in_files(ElementId(int(linkedElementId)))
                    if tempLinkedDocData is None: continue
                    link_instance = tempLinkedDocData[1]
                    ...
                    if categoryReferece and categoryReferece.BuiltInCategory == BuiltInCategory.OST_GenericModel:
                        ...
                        linked_rotation = math.atan2(transform.BasisX.Y, transform.BasisX.X)
                        point = XYZ(transform.OfPoint(linkedPoint).X, transform.OfPoint(linkedPoint).Y, transform.OfPoint(linkedPoint).Z)
                    ...
                    try:
                        if link_doc != None and transform != None:
                            instanceNewHole = self.doc.Create.NewFamilyInstance(point, targetSymbol, StructuralType.NonStructural)
                ...
                #             instanceNewHole = self.doc.GetElement(new_instance)
                            ...
                            for p in instanceNewHole.Parameters:
                                try:
                                    if p and not p.IsReadOnly:
                                        pName = p.Definition.Name
                                        if pName in dataParentParameters.keys():
                                            hostParameter = dataParentParameters[pName]
                                            storageType = hostParameter['STORAGE_TYPE']
                                            value       = hostParameter['VALUE']
                            ...
                            local_rotation = linkedElement.Location.Rotation
                            rotation_axis = Line.CreateBound(instanceNewHole.Location.Point, instanceNewHole.Location.Point + XYZ(0, 0, 1))
                            ElementTransformUtils.RotateElement(self.doc, instanceNewHole.Id, rotation_axis, local_rotation + linked_rotation)

                            locZData = self.parent.getAbsolutLocation(instanceNewHole, self.ZHCodeType)
                            parameter = instanceNewHole.LookupParameter('ADSK_Размер_Отметка расположения')
                            if parameter:
                                parameter.Set(locZData[0])

                            parameterABS = instanceNewHole.LookupParameter('ZH_Абсолютная отметка')
                            if parameterABS:
                                parameterABS.Set(locZData[1])

                            parameterNote = instanceNewHole.LookupParameter('ADSK_Примечание')
                            if parameterNote:
                                parameterNote.Set("КР")
...
class SetAimMark(IExternalEventHandler):
    def __init__(self, parent):
        ...
        self.uidoc = uidoc
        self.elems = None
        self.userAppName = userNameApp

    def Execute(self, commandData):
        import json
        import os
        import codecs
        with codecs.open(os.path.join(self.parent.dirnameFile, r'TempData\ExceptionIds.json'), 'r', encoding='utf-8-sig') as file: #TODO затычка которая нужна чтобы пропускать id конфликтных элементов
            temp_exception_ids = json.load(file)
        ...
        docTitle = self.doc.Title.replace("_" + self.userAppName, '').replace('_отсоединено', '')
        ...
        if self.elems:
            ...
            for hole in self.elems:
                if temp_exception_ids.get(docTitle, None) and hole.Id.ToString() in temp_exception_ids[docTitle]:
                    continue
                try:
                    zhCodeTypeDigit = self.parent.getZHCodeTypeDigitValue(hole)
                    if zhCodeTypeDigit:
                        if zhCodeTypeDigit > 99 and zhCodeTypeDigit <100:
                            locZData = self.parent.getAbsolutLocation(hole, zhCodeTypeDigit)
                            parameter = hole.LookupParameter('ADSK_Размер_Отметка расположения')
                            if parameter:
                                parameter.Set(locZData[0])
                            parameterABS = hole.LookupParameter('ZH_Абсолютная отметка')
                            if parameterABS:
                                parameterABS.Set(locZData[1])
...
class SetSortPosition(IExternalEventHandler):
    def __init__(self, parent):
        self.parent = parent
        self.doc = doc
        self.uidoc = uidoc
        self.OnPlaneSquare = False
        self.OnLevelSquare = False
        self.OnPlaneCircle = False
        self.OnLevelCircle = False
    ...
    def Execute(self, commandData):
        ...
        try:
            data = ["ADSK_ОбобщеннаяМодель_Отверстие{}".format(i[1]) for i in zip([self.OnPlaneSquare, self.OnLevelSquare, self.OnPlaneCircle, self.OnLevelCircle],
                                                            ["Прямоугольное_НаПлоскости", "Прямоугольное_НаУровне", "Круглое_НаПлоскости", "Круглое_НаУровне"]) if i[0]]
        except Exception as e:
        ...
        try:
            def get_value(elem, paramName):
                value = None
                param = elem.LookupParameter(paramName)
                if param is None:
                    param = elem.Symbol.LookupParameter(paramName)
                ...
                elif paramName in ['ADSK_Размер_Диаметр', 'ADSK_Отверстие_Высота', 'ADSK_Отверстие_Ширина']:
                    value = 0
                else:
                    value = None
                return value
            ...
            def create_dict_key(elem, paramNames):
                try:
                    data = orderedDict.setdefault(elem.Id, {})
                    for paramName in paramNames:
                        value = get_value(elem, paramName)
                        data[paramName] = value
                except Exception as e:
                    print(e)

            selectedElems = [i for i in FilteredElementCollector(self.doc).OfCategory(BuiltInCategory.OST_GenericModel).WhereElementIsNotElementType().ToElements()
                             if i.get_Parameter(BuiltInParameter.ELEM_FAMILY_PARAM).AsValueString() in data]

            if len(selectedElems) == 0: return

            schedule = self.doc.ActiveView
            if not isinstance(schedule, ViewSchedule): return

            definition = schedule.Definition
            resultParams = []
            for i in range(definition.GetFieldCount()):
                param = definition.GetField(i)
                if param.IsHidden:
                    continue

                try:
                    paramName = self.doc.GetElement(param.ParameterId).Name
                    resultParams.append(paramName)
                except:
                    continue

            for elem in selectedElems:
                create_dict_key(elem, resultParams)

            result = []
            for k, v in orderedDict.items():
                sorted1 = round(v['ADSK_Отверстие_Ширина'] * 304.8 * 10000 + v['ADSK_Отверстие_Высота'] * 304.8 + v['ADSK_Размер_Толщина основы'] * 304.8, 4) if all([v['ADSK_Отверстие_Ширина'], v['ADSK_Отверстие_Высота']]) else 0
                sorted2 = round(v['ADSK_Размер_Диаметр'] * 304.8 * 10000 + v['ADSK_Размер_Толщина основы'] * 304.8, 4) if all([v['ADSK_Размер_Диаметр']]) else 0
                sorted3 = v['ADSK_Отверстие_Функция']
                sorted4 = round(v['ADSK_Размер_Отметка расположения'],2) * 304.8

                result.append([k, sorted1, sorted2, sorted3, sorted4])
            ...
            t = Transaction(self.doc, 'updatePosition')
            t.Start()
            for i in result:
                elId = i[0]
                sortData = tuple(i[1:])
                if tempFlag == sortData:
                    pass
                else:
                    tempFlag = sortData
                    positionNumber += 1
                self.doc.GetElement(elId).LookupParameter('ADSK_Позиция').Set(str(positionNumber))
...
class CollisionForm(Form):
    def __init__(self):
        ...
        self.handlerSetSortPosition = SetSortPosition(self)
        self.external_eventSetSortPosition = ExternalEvent.Create(self.handlerSetSortPosition)
    ...
    def mainPanel(self):
        ...
        # Меняем статус "смещение" у элемента
        imageChangeChangeMarkAim = Image.FromFile(os.path.join(dirnameFileImages, r'markAim.png'))
        self.btnChangeMarkAim = Button(Location=Point(475, panelMainBottomLeft.Height - 22),
                                              Size=Size(30, 30),
        ...
        """-------------------Обновить позиции отверстий--------------------------------------"""
        labelCopy = Label(Text='Позиции',
                          Size=Size(65, 25),
                          Location=Point(520, panelMainBottomLeft.Height - 50),
                          TextAlign = ContentAlignment.MiddleCenter)
        panelMainBottomLeft.Controls.Add(labelCopy)

        # Меняем статус "смещение" у элемента

        imageChangeChangeMarkAim = Image.FromFile( os.path.join(dirnameFileImages, r'sortIconEnabled2.png'))
        self.btnUpdateSortPosition = Button(Location=Point(540, panelMainBottomLeft.Height - 22),
                                       Size=Size(30, 30),
                                       BackgroundImage=imageChangeChangeMarkAim,
                                       BackgroundImageLayout=ImageLayout.Zoom,
                                            Enabled = False)
        self.btnUpdateSortPosition.Click += self.updateSortPositionValue
        panelMainBottomLeft.Controls.Add(self.btnUpdateSortPosition)
        ...
        try:
            ...
            if self.doc.GetElement(element_id) is not None and isinstance(self.doc.GetElement(element_id), FamilyInstance):
                ...
                for linked_instance, link_doc in linked_documents:
                    element = link_doc.GetElement(element_id)
                    if element:
                        if element.Category and element.Category.BuiltInCategory == BuiltInCategory.OST_GenericModel:
                            return element, linked_instance
            return None
        except Exception as e:
            print(e)
    ...
    def onCellClickHole(self, sender, event):
        ...
        try:
            ...
            if self.row_index >= 0:
                ...
                actual_Status = row.Cells["Actual"].Value
                ...
                if elem1 is not None:
                    if actual_Status == 'Нет в проекте':
                        text1 = ('Элемент 1:\nОтсутсвует в текущем проекте ({}....)\n'
                                 'Чтобы отверстие появилось в проекте, нужно его скопировать.').format(
                            self.doc.Title[:20])
                    else:
                        elem1 = elem1[0]
                    ...
                    if actual_Status == 'Нет в связи':
                        text1 = ('Элемент 1:\nНе обнаружено связанного файла, с которого можно получить данное отверстие.\n'
                                 'Убедитесь, что нужные смежные файлы загружены.')
                    else:
                        text1 = 'Элемент 1:\nБез описания'
    ...
    def getAbsolutLocation(self, hole, zhCodeTypeDigit):
        from Autodesk.Revit.DB import FilteredElementCollector as FEC, BuiltInCategory as BC
        valueToWrite, valueToWriteABS = None, None
        zLocation        = hole.Location.Point.Z
        absPoint         = FEC(self.doc).OfCategory(BC.OST_ProjectBasePoint).WhereElementIsNotElementType().ToElements()[0]
        absLocDocZ       = absPoint.Position.Z
        absLocDocSharedZ = absPoint.SharedPosition.Z

        try:
            if zhCodeTypeDigit == 99.03:  ###Круглый на плоскости
                elemSize = hole.LookupParameter('ADSK_Размер_Толщина основы')
                valueToWrite = zLocation - (elemSize.AsDouble()) - absLocDocZ
                valueToWriteABS = absLocDocSharedZ + (zLocation - (elemSize.AsDouble()) - absLocDocZ)

            elif zhCodeTypeDigit == 99.04:  ###Круглый на уровне
                elemSize = hole.LookupParameter('ADSK_Размер_Диаметр')
                valueToWrite = zLocation - absLocDocZ
                valueToWriteABS = absLocDocSharedZ + (zLocation - absLocDocZ)

            elif zhCodeTypeDigit == 99.01:  ###Прямоугольный на плоскости
                elemSize = hole.LookupParameter('ADSK_Отверстие_Высота')
                valueToWrite = zLocation - (elemSize.AsDouble() / 2) - absLocDocZ
                valueToWriteABS = absLocDocSharedZ + (zLocation - (elemSize.AsDouble() / 2) - absLocDocZ)

            elif zhCodeTypeDigit == 99.02:  ###Прямоугольный на уровне
                elemSize = hole.LookupParameter('ADSK_Отверстие_Высота')
                valueToWrite = zLocation - (elemSize.AsDouble() / 2) - absLocDocZ
                valueToWriteABS = absLocDocSharedZ + (zLocation - (elemSize.AsDouble() / 2) - absLocDocZ)

            # print(hole.Id)
            # print(valueToWrite * 304.8)
            # print(valueToWriteABS * 304.8)


            return [round(valueToWrite * 304.8, 1) / 304.8 , round(valueToWriteABS * 304.8, 1) / 304.8]
        except Exception as e:
    ...
    def changeMarkAimHoles(self, sender, event):
        ...
        try:
            ...
            print('changeMarkAimHoles : {}'.format(e))
            return
        self.btnUpdateSortPosition.Enabled = True
        imageChangeChangeMarkAim = Image.FromFile(os.path.join(self.dirnameFileImages, r'sortIcon1.png'))
        self.btnUpdateSortPosition.BackgroundImage = imageChangeChangeMarkAim


    def updateSortPositionValue(self, sender, event):
        from Autodesk.Revit.DB import ElementId, BuiltInParameter, FilteredElementCollector, BuiltInCategory
        try:
            self.handlerSetSortPosition.OnPlaneSquare = self.jsonSettings["OnPlaneSquare"]
            self.handlerSetSortPosition.OnLevelSquare = self.jsonSettings["OnLevelSquare"]
            self.handlerSetSortPosition.OnPlaneCircle = self.jsonSettings["OnPlaneCircle"]
            self.handlerSetSortPosition.OnLevelCircle = self.jsonSettings["OnLevelCircle"]

            self.external_eventSetSortPosition.Raise()
        except Exception as e:
    ...
    def getZHCodeTypeDigitValue(self, elem):
        ...
        if zhCodeTypeDigitParam:
            return zhCodeTypeDigitParam.AsDouble()
        else: return None
            ...
            for elementIdStr, properties in self.json_file.items():
                ...
                if elementIdStr not in elementsInProject:
                    ...
                    if linkedResult == None:
                        ...
                        currentPointElement = mainElemInProject.get_Parameter(BuiltInParameter.ALL_MODEL_INSTANCE_COMMENTS).AsValueString().split('_')[3]
