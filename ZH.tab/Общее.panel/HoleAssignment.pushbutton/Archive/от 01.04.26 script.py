...
class CopyElementsHandler(IExternalEventHandler):
    ...
    def Execute(self, commandData):
        ...
        paramToMiss = ['Семейство', 'Тип', 'Семейство и типоразмер']
        def get_all_instance_parameters(linked_Element):
            ...
            for p in linked_Element.Parameters:
                if p:
                    if p.Definition.Name not in paramToMiss:
        ...
        try:
            ...
            if self.elements != None:
                for linkedElementId in self.elements:
                    ...
                    #             self.doc.Regenerate()
                    #
                    #             locZData = self.parent.getAbsolutLocation(new_instance, self.ZHCodeType)
                    #             parameter = new_instance.LookupParameter('ADSK_Размер_Отметка расположения')
                    #             if parameter:
                    #                 parameter.Set(locZData[0])
                    #             parameterABS = new_instance.LookupParameter('ZH_Абсолютная отметка')
                    #             if parameterABS:
                    #                 parameterABS.Set(locZData[1])
                    #
                    #
