...
class IsolateElemsHandler(IExternalEventHandler):
    ...
    def Execute(self, commandData):
        ...
        try:
            ...
            if self.uidoc.ActiveView.Name == 'ZH_3DView_Collision' and len(element_ids) > 0:
    ...
    def Execute(self, commandData):
        try:
            ...
            if len(intersection_points) > 0:
                ...
                view_3d = None
                current_3D_view_in_doc = [view for view in FilteredElementCollector(self.doc).OfClass(View).WhereElementIsNotElementType().ToElements() if view.Name == 'ZH_3DView_Collision']
                if len(current_3D_view_in_doc) > 0:
                    ...
                    view_3d.Name = 'ZH_3DView_Collision'
...
class CollisionForm(Form):
    ...
    def mainPanel(self):
        ...
        imageMatrixCollision = Image.FromFile(os.path.join(dirnameFile, r'Images\web.png'))
        ...
        imageUpdateCollision = Image.FromFile(os.path.join(dirnameFile, r'Images\sinchronize.png'))
        ...
        imageSearchFile = Image.FromFile(os.path.join(dirnameFile, r'Images\searchFile.png'))
        ...
        self.dataGridViewChecks = DataGridView(Location = Point(0,0),
                                               ...
                                               Enabled = False)
        columns = columns = [
            ('Имя проверки', 'NameCheck',48.5,True),
        ...
        columns = columns = [
            ...
            ('№ Конфликта', 'ConflictName', 20, True),
        ...
        panelGetViewConflict = Panel(Size = Size(120, 30),
        ...
        imageSearchFile = Image.FromFile(os.path.join(dirnameFile, r'Images\show.png'))
        ...
        imageSearchFile = Image.FromFile(os.path.join(dirnameFile, r'Images\zoom.png'))
        ...
        imageSearchFile = Image.FromFile(os.path.join(dirnameFile, r'Images\isolate.png'))
        ...
        imageSelectElems = Image.FromFile(os.path.join(dirnameFile, r'Images\select.png'))
    ...
    def onCellClickConflict(self, sender, event):
        ...
        try:
            ...
            if row_index >= 0:
                ...
                name_Conflict = row.Cells["ConflictName"].Value
                try:
                    pathToImage = os.path.join('\\'.join(self.folderFile.FileName.split('\\')[:-1]), self.rowConflicts[name_Conflict][-3])
                    self.pictureBox.Image = Image.FromFile(pathToImage)
                except Exception as e:
                    pathToImage = os.path.join(self.dirnameFile,r'Images\notFoundImage.png')
                    self.pictureBox.Image = Image.FromFile(pathToImage)

                self.labelInfoElem1.Name = self.rowConflicts[name_Conflict][2]
                self.labelInfoElem2.Name = self.rowConflicts[name_Conflict][3]
                ...
                if elem1 is not None:
                    ...
                    doc1  = elem1.Document.Name
                else:
                    text1 = 'Элемент 1:\nОтсутсвует в проекте'

                if elem2 is not None:
                    cat2 = elem2.Category.Name
                    fam2 = elem2.get_Parameter(BuiltInParameter.ELEM_FAMILY_PARAM).AsValueString()
                    typ2 = elem2.get_Parameter(BuiltInParameter.ELEM_TYPE_PARAM).AsValueString()
                    text2 = 'Элемент 2:\nКат:{}\nСем:{}\nТип:{}'.format(cat2, fam2, typ2)
                    doc2 = elem2.Document.Name
                    # toolTip.SetToolTip(self.labelInfoElem1, 'Нажмите, чтобы выбрать')
                else:
                    text2 = 'Элемент 2:\nОтсутсвует в проекте'
