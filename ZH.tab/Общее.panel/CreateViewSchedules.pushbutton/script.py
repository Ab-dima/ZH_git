...
pathWhoDeteleSets        = os.path.join(dirnameFile, 'whoDeteleSets.json')
...
class RunCreateSchedule(IExternalEventHandler):
    def __init__(self):
        self.doc = doc
        self.parent = None
        self.uidoc = uidoc
        self.app = app
        self.formDublicateSheets = ToDeleteSheetForm()
        self.infoForm = InformationForm('')

    def Execute(self, commandData):
        from Autodesk.Revit.DB import ViewSheet, FilteredElementCollector, BuiltInCategory, BuiltInParameter, Transaction, \
        ScheduleSheetInstance, ElementClassFilter, FamilyInstance, ParameterValueProvider, FilterStringEquals, FilterStringRule, ElementParameterFilter, ElementId, \
        UnitUtils, UnitTypeId, XYZ

        from System.Collections.Generic import List

        def get_type_by_name_to_firstElement(familyParam, typeParam):
            allFamilyTypeSheets = FilteredElementCollector(self.doc).OfCategory(
        ...
        def createInfoForm(textInfo, x = None, y = None):
            self.parent.Hide()
            try:
                self.infoForm.infoLabel.Text = textInfo
                if x and y:
                    self.infoForm.Width = x
                    self.infoForm.Height = y
                self.infoForm.ShowDialog()
            except Exception as e:
                print(e)
                self.parent.Show()
                return None
        ...
        errorSchedules = []
        for vsName in self.parent.selectedViewScghedules:
            if vsName not in self.parent.viewSchedules:
        ...
        allTitleBlockNumbers = [tb for tb in FilteredElementCollector(self.doc).OfCategory(BuiltInCategory.OST_Sheets).WhereElementIsNotElementType().ToElements()]
        lstDuplicated = []
        ...
        if len(lstDuplicated) > 0:
            self.parent.Hide()
            try:
                self.formDublicateSheets.lstSheets = lstDuplicated
                self.formDublicateSheets.populateBuiltInPanel()
                self.formDublicateSheets.ShowDialog()
            except Exception as e:
                print(e)
                self.parent.Show()
                pass
        ...
        startNumberSheet = 1 if len(self.parent.textBoxStartSheet.Text) < 1 else int(self.parent.textBoxStartSheet.Text)
        nameSheets       = 'Без имени' if len(self.parent.textBoxNameSheet.Text) < 1 else self.parent.textBoxNameSheet.Text


        chechBoxDiffSheet = self.parent.checkBoxDiffSheets.Checked
        typeSheet_1       = self.parent.comboboxTypeSheet_1.Text
        typeSheet_2       = self.parent.comboboxTypeSheet_2.Text
        ...
        if typeSheet_1 is None:
            ...
            if typeSheet_1 is None:
                ...
                return None
            segmentsSize_1 = 0 if len(self.parent.textBoxSize_1.Text) == 0 else int(self.parent.textBoxSize_1.Text)
        ...
        if chechBoxDiffSheet:
            if typeSheet_2 is None:
                ...
                if typeSheet_2 is None:
                    ...
                    return None
                segmentsSize_2 = 0 if len(self.parent.textBoxSize_2.Text) == 0 else int(self.parent.textBoxSize_2.Text)
        ...
        try:
            selectedViewSchedules = [get_view_by_name_to_element(viewName) for viewName in self.parent.selectedViewScghedules]
        ...
        sizeDevyatigrafka = 32 if self.parent.checkBoxDevyatigrafka.Checked else 0
        ...
        newUsingSheet = ViewSheet.Create(self.doc, typeSheet_1)
        try:
        ...
        try:
            for viewSc in selectedViewSchedules:
                ...
                scSheetInstance = ScheduleSheetInstance.Create(self.doc,
                                                               newUsingSheet.Id,
                                                               viewSc.Id,
                                                               XYZ(0,0,0))

                self.doc.Regenerate()
                bbox = scSheetInstance.get_BoundingBox(self.doc.GetElement(scSheetInstance.OwnerViewId))
                bbMax = bbox.Max
                bbMin = bbox.Min
                ...
                if segmentCount == 1:
                    try:
                        if stPoint.ToString() != xyzPozition.ToString():
                            ...
                            try:
                                ...
                                if chechBoxDiffSheet and counterCreatedSheets == 1:
                                    typeSheet = typeSheet_2
                                newSheet = ViewSheet.Create(self.doc, typeSheet)
                                createSheetsLst.append(newSheet)
                            except Exception as e:
                                print(e)

                            counterCreatedSheets += 1
                        newScSheetInstance = ScheduleSheetInstance.Create(self.doc,
                                                                          newSheet.Id,
                                                                          viewSc.Id,
                                                                          stPoint.Add(correctionPt))

                        bboxSegment = newScSheetInstance.get_BoundingBox(self.doc.GetElement(newScSheetInstance.OwnerViewId))
                    ...
                    try:
                        ...
                        lastScheduleToCheck = ScheduleSheetInstance.Create(self.doc,
                                                                           newUsingSheet.Id,
                                                                           viewSc.Id,
                                                                           XYZ(0, 0, 0),
                                                                           segmentCount - 1)
                        bboxLastSegment = lastScheduleToCheck.get_BoundingBox(
                            self.doc.GetElement(lastScheduleToCheck.OwnerViewId))
                        ...
                        for i in range(segmentCount):
                            ...
                            if stPoint.ToString() != xyzPozition.ToString():
                                ...
                                try:
                                    ...
                                    if chechBoxDiffSheet and counterCreatedSheets == 1:
                                        typeSheet = typeSheet_2
                                    newSheet = ViewSheet.Create(self.doc, sheetType)
                                    createSheetsLst.append(newSheet)
                                except Exception as e:
                                    print(e)
        ...
        try:
            t = Transaction(self.doc, 'renameSheets')
            t.Start()
            ...
            for createdSheet in createSheetsLst:
                try:
                    ...
                    if self.parent.checkBoxDevyatigrafka.Checked:
                        try:
                            sheetInstance = self.doc.GetElement(createdSheet.GetDependentElements(ElementClassFilter(FamilyInstance))[0])
...
class NameOrRenameForm(Form):
    ...
    def controlPanel(self):
        from System.Windows.Forms import Cursor, Button, Button
...
class ToDeleteSheetForm(Form):
    ...
    def controlPanel(self):
        from System.Windows.Forms import Cursor, Button
        from System.Drawing import Point, Size
        def mouseMove(sender, event):
            if self.dragging:
                dif = Point.Subtract(Cursor.Position, Size(self.cursorLocation))
    ...
    def deleteAll(self, sender, event):
        ...
        from System.Drawing import Color
        t = Transaction(self.doc, 'deleteAll')
        t.Start()
        self.doc.Regenerate()
        elemDeletedElems = []
        ...
        for elemId in self.lstSheets:
            try:
                if self.doc.ActiveView.Id.ToString() == elemId:
                    ...
                    continue
                self.doc.Delete(ElementId(int(elemId)))
    ...
    def deleteSheet(self, sender , event):
        ...
        from System.Drawing import Color
        t = Transaction(self.doc, 'delete titleBlock')
        t.Start()
        self.doc.Regenerate()
        ...
        for control in parentPanel.Controls:
            try:
                if isinstance(control, Label):
                    if self.doc.ActiveView.Id.ToString() == control.Name:
                        self.labelInfo.Text = '! Перейдите на иной вид, для удаления листа!'
                        self.labelInfo.BackColor = Color.Red
                        self.labelInfo.ForeColor = Color.White
                        continue
                    controlToDel = control
                    self.doc.Delete(ElementId(int(control.Name)))
    ...
    def populateBuiltInPanel(self):
        ...
        if len(self.lstSheets) > 0:
            for sheetId in self.lstSheets:
                ...
                titleBlock  = self.doc.GetElement(ElementId(int(sheetId)))
...
class CreateSchedulesForm(Form):
    def __init__(self):
        ...
        self.pathWhoDeteleSets = pathWhoDeteleSets
        ...
        self.formToNameOrRename = NameOrRenameForm('SETNAME')
        self.formToRename       = NameOrRenameForm('RENAME')

        self.handlerRunCreateSchedule = RunCreateSchedule()
        self.external_eventRunCreateSchedule = ExternalEvent.Create(self.handlerRunCreateSchedule)
    ...
    def controlPanel(self):
        ...
        self.controlsIgnor.append(self.panelTop)
        self.Controls.Add(self.panelTop)
        self.Controls.SetChildIndex(self.panelTop, 0)
        self.panelTop.MouseUp += mouseUp
        self.panelTop.MouseDown += mouseDown
        self.panelTop.MouseMove += mouseMove
    ...
    def bottomPanel(self):
        ...
        self.Controls.SetChildIndex(self.panelBottomInfo, 0)
        ...
        self.panelBottom.Controls.SetChildIndex(separator,0)
        ...
        self.panelBottom.Controls.SetChildIndex(self.panelBottomLeft, 0)
        ...
        self.castomSeparateBottom2 = Panel(Size=Size(self.panelBottomLeft.Height*50,1),
    ...
    def mainPanel(self):
        ...
        deleteSetBtn.Click += self.triggerPopulateComboBoxSet
        ...
        def renameSet(sender, event):
            ...
            if setToRename in self.startedSets.keys() and setToRename != '<Нет>':
                self.Hide()
                try:
                    self.formToRename.ShowDialog()
                    if self.formToRename.backStatus == True:
                        self.Show()
                        return None
                    renameText = self.formToRename.resultName
                    print(renameText)
                except:
                    self.Show()
                    return None
                ...
                with open(self.pathSets, 'w') as file:
    ...
    def bordersPanel(self):
        ...
        sepRight = Panel(Name='SideBorders',
                         Width=1,
                         Dock=DockStyle.Right)
        self.Controls.Add(sepRight)
        self.Controls.SetChildIndex(sepRight, 0)
        """--------------------------------------------------------------"""

    def triggerPopulateComboBoxSet(self, sender, event):
        self.populateComboBoxSet()

    def populateComboBoxSet(self):
        self.comboBoxSets.Items.Clear()
        for s in self.startedSets.keys():
            self.comboBoxSets.Items.Add(s)

    def setNameSets(self):
        import json
        self.Hide()
        try:
            self.formToNameOrRename.ShowDialog()
        except Exception as e:
            print(e)
            self.Show()
            return None
        ...
        if not self.formToNameOrRename.backStatus:
            try:
                newName = self.formToNameOrRename.resultName
    ...
    def populateTreeViewSchedule(self):
        ...
        for vS in self.viewSchedules:
            if vS not in self.selectedViewScghedules and '<Ведомость изменений>' not in vS:
    ...
    def deleteSet(self, sender, event):
        ...
        if setToDel in self.startedSets.keys() and setToDel != '<Нет>':
            ...
            try:
                ...
                dataWhoDeletes = []
                if os.path.exists(self.pathWhoDeteleSets):
                    with open(self.pathWhoDeteleSets, 'r') as file:
                        dataWhoDeletes = json.load(file)

                dataWhoDeletes.append('{} delete {}'.format(self.userName, setToDel))
                with open(self.pathWhoDeteleSets, 'w') as file:
                    json.dump(dataWhoDeletes, file)
    ...
    def runScript(self, sender, event):
        try:
            self.handlerRunCreateSchedule.doc = self.doc
            self.handlerRunCreateSchedule.parent = self
            self.external_eventRunCreateSchedule.Raise()
        except Exception as e:
            print(e)
