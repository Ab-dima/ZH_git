...
if userName not in ['abankin.dp', 'kaskiv.ri', 'elaev.sa', 'pozdeev', 'berezina.yi']:
...
class CTZ(Form):
    def __init__(self):
        ...
        try:
            ...
            required_keys = ['CHECKLinkDocs', 'IGNORELinkDocs']
    ...
    def middlePanel(self):
        ...
        self.btnDownloadData = Button(Location=Point(335, 8),
                                              Size=Size(30, 30),
                                              BackgroundImage=imageChangeLocation,
                                              BackgroundImageLayout=ImageLayout.Zoom)
        self.btnDownloadData.Click += self.getData
        ...
        label = Label(Text='ПРОВЕРИТЬ:',
                               Size=Size(90, 22),
                               Location=Point(mainPanel.Width / 4-80, mainPanel.Height - 120),
                               TextAlign=ContentAlignment.MiddleCenter,
                               Enabled=False,
        ...
        label = Label(Text='ПРОВЕРИТЬ ВЫБРАННЫЙ В:',
                      Size=Size(200, 22),
                      Location=Point(mainPanel.Width / 2 - 90, mainPanel.Height - 120),
                      TextAlign=ContentAlignment.MiddleCenter,
                      Enabled=False,
                      Font=Font('ISOCPEUR', 12))
        mainPanel.Controls.Add(label)
        ...
        self.btnCheck = Button(Name = 'CHECK1',
                               Text = 'Модель',
                               Size=Size(80, 22),
                               Location=Point(mainPanel.Width / 4-75, mainPanel.Height -95),
                               TextAlign = ContentAlignment.MiddleCenter,
                               Enabled = False,
        ...
        self.btnCheck2 = Button(Name = 'CHECK2',
                                Text='текущем\nфайле',
                               Size=Size(80, 40),
                               Location=Point(mainPanel.Width / 2 - 85, mainPanel.Height - 100),
                               TextAlign=ContentAlignment.MiddleCenter,
                               Enabled=False,
                                Font = Font('ISOCPEUR', 11))
        ...
        self.btnCheck3 = Button(Name='CHECK2',
                                Text='связянном\nфайле',
                                Size=Size(80, 40),
                                Location=Point(mainPanel.Width / 2 + 20, mainPanel.Height - 100),
                                TextAlign=ContentAlignment.MiddleCenter,
                                Enabled=False,
                                Font=Font('ISOCPEUR', 11))
        ...
        self.btnSend = Button(Text='ОТПРАВИТЬ',
                                            Size=Size(95, 30),
                                            Location=Point((mainPanel.Width / 4) + (mainPanel.Width/ 2)+5, mainPanel.Height -105),
                                            TextAlign=ContentAlignment.MiddleCenter,
                              Enabled = False)
        ...
        for i in [160, 390]:
            sepVertical = Panel(Size=Size(1, 50),
                              Location=Point(i, mainPanel.Height - 115))
        ...
        """----------------------Setting---------------------------"""
        self.panelSettings = Panel(Size = Size(self.Width - 22,408),
        ...
        label = Label(Location=Point(10, 95),
                             Size=Size(300, 30),
                             Text='Игнорировать св.файлы содержащие:')
        self.panelSettings.Controls.Add(label)

        self.ignoreLinkedFilesTextBox = TextBox(Location=Point(10, 125),
                                        Size=Size(300, 30),
                                        Text = ' ' if self.dataSetting is None else self.dataSetting['IGNORELinkDocs'])
        self.panelSettings.Controls.Add(self.ignoreLinkedFilesTextBox)
        ...
        self.autoScrollingParamsData = CheckBox(Location=Point(10, 35),
                                                  Size=Size(500, 30),
                                                  Checked=False if self.dataSetting is None else self.dataSetting[
                                                      'CHECKLinkDocs'],
    ...
    def getTrueElems(self, sender, event):
        ...
        try:
            ...
            selectedNode = '{}||{}'.format(self.nodSel.Name,' '.join(self.nodSel.Text.split(' ')[1:]))
            elemIds = [ElementId(int(i)) for i in self.sendChecksResult[selectedNode]['trueElems']]
    ...
    def checkData(self,sender, event):
        ...
        try:
            ...
            for i in FilteredElementCollector(self.doc).OfCategory(BuiltInCategory.OST_RvtLinks).WhereElementIsNotElementType().ToElements():
                ...
                ignorePrefixLinkedFiles = self.ignoreLinkedFilesTextBox.Text.split(';')
                if len(ignorePrefixLinkedFiles) != 0 and '' not in ignorePrefixLinkedFiles:
                    try:
                        titleLinkDoc = linkDoc.Title
                    except:
                        continue
                    for prefix in ignorePrefixLinkedFiles:
                        if prefix.strip() in titleLinkDoc:
                            break
                    else:
                        self.linkedDocuments.append(linkDoc)
                else:
                    self.linkedDocuments.append(linkDoc)
                ...
                try:
                    ...
                    for data in dataResult[3:]:
                        ...
                        if status in ['TRUE', 'FALSE']:
                            ...
                            self.result[nameCheck] = OrderedDict()
                            self.result[nameCheck]['Status'] = status
                            self.result[nameCheck]['Category'] = [category]
                            self.result[nameCheck]['CategoryRus'] = [categoryRus]
                            self.result[nameCheck]['Checks'] = OrderedDict()
                            self.result[nameCheck]['Checks']['Check{}'.format(counterChecks)] = OrderedDict()
                            self.result[nameCheck]['Checks']['Check{}'.format(counterChecks)]['check{}'.format(counter_checks)] = {'Parameter': paramCheck,
                            ...
                            if status == '':
                                if len(data[columnCategory]) != 0:
                                    self.result[flagName]['Category'].append(category)
                                    self.result[flagName]['CategoryRus'].append(categoryRus)

                                if paramCheck in ['ИЛИ', 'И']:
                                    self.result[flagName]['Checks'][paramCheck] = {paramCheck:None}
                                    counterChecks += 1
                                    counter_checks = 0
                                    continue
                                ...
                                if paramCheck != '' or operatorCheck != '' or valueCheck != '':
                                    counter_checks += 1
                                    if 'Check{}'.format(counterChecks) not in self.result[flagName]['Checks']:
                                        self.result[flagName]['Checks']['Check{}'.format(counterChecks)] = {'check{}'.format(counter_checks) : {'Parameter': paramCheck,
                                                                                                       'Operator': operatorCheck,
                                                                                                       'Value': valueCheck}}
                                    else:
                                        self.result[flagName]['Checks']['Check{}'.format(counterChecks)]['check{}'.format(counter_checks)] = {'Parameter': paramCheck,
                    ...
                    #     print("Category = {}".format((result[r]['Category'])))
                    ...
                    for r in self.result:
                        try:
                            nameCheck = r
                            resultText = []
                            if self.result[r]['Category']:
                                resultText.append('Категория: {}'.format(';'.join(str(i) for i in self.result[r]['CategoryRus'])))
                            for ch1 in self.result[r]['Checks']:
                                for ch2 in self.result[r]['Checks'][ch1]:
                                    if ch2 in ['ИЛИ', 'И']:
                                        resultText.append('{}'.format(ch2))
                                    else:
                                        parameter = self.result[r]['Checks'][ch1][ch2]['Parameter']
                                        operator = self.result[r]['Checks'][ch1][ch2]['Operator']
                                        value = self.result[r]['Checks'][ch1][ch2]['Value']
                                        resultText.append('{}  {}  {}'.format(parameter, operator, value))
                                self.toolTipsInfo[nameCheck] = resultText
                        except:
                            pass
            ...
            try:
                ...
                for nameCheck in self.result.keys():
                    ...
                    if sender.Name == 'CHECK2':
                        ...
                        if selectedNameCheck:
                            selectedNameCheckText = '{}||{}'.format(selectedNameCheck.Name,' '.join(selectedNameCheck.Text.split()[1:]))
                            if selectedNameCheckText == ' '.join(nameCheck.split()):
                                self.Hide()
                                checkSelElem = None
                                try:
                    ...
                    for checkKey, checkValue in checks.items():
                        ...
                        for elem in elems:
                                    ...
                                    for materialCheck in materialDictChecks.keys():
                                        for materialLayer in elemMaterials:
                                            ...
                                            for matCheck in materialDictChecks[materialCheck]:
                                                ...
                                                resultMaterialBool[mParam] = _boolResult
                            ...
                            if False in centerBoolCheck:
                                ...
                                self.resultAllChecks['trueElems'].append(elem.Id.ToString())
                ...
                # print('*'*20)
                # print('lenght = {}'.format(len(self.sendChecksResult)))
                # for k,v in self.sendChecksResult.items():
                #     print('{}: {}'.format(k,v))
    ...
    def updateTreeViewNodes(self):
        try:
            ...
            for check, boolStatus in self.sendChecksResult.items():
                ...
                status = self.result[check]['Status']
                nodeName = check.split('||')[0]
                nodeText = '({}/{}) {}'.format(passed_count_currentFile, passed_count_linkFile, check.split('||')[1])
                if self.searchRule.Text.lower() in nodeText.lower() or self.searchRule.Text == '':
    ...
    def getData(self, sender, event):
        ...
        try:
            ...
            with open(os.path.join(self.dirnameFile, r'SubproccesScripts\sendDataNameGoogleTable.json'), 'r') as file:
                data = json.load(file)
            data['NameTable'] = self.textBoxTableName.Text
            with open(os.path.join(self.dirnameFile, r'SubproccesScripts\sendDataNameGoogleTable.json'), 'w') as file:
                json.dump(data, file)
    ...
    def closeForm(self, sender, event):
        import json
        try:
            data = {'CHECKLinkDocs': self.ignoreLinkedFilesCheckBox.Checked,
                    'IGNORELinkDocs': self.ignoreLinkedFilesTextBox.Text}
            with open(self.tempPath, 'w') as file:
                json.dump(data, file)
        except:
            print('Ошибка при записи настроек')
        self.Close()
