...
class CTZ(Form):
    def __init__(self):
        ...
        try:
            ...
            required_keys = ['CHECKLinkDocs', 'AUTOResizeChecks', 'Projects', 'ColorRGB']
    ...
    def controlPanel(self):
        ...
        self.btnGetActualChecks = Button(Name = "GETCHECKS",
                                 ...
                                 Location=Point(95, 3),
                                 Size=Size(25, 25),
                                         Visible = True if self.userName in ['abankin.dp', 'pozdeev', 'hlybov.se'] else False)
    ...
    def overrideSelectedElems(self, sender, event):
        from Autodesk.Revit.DB import BuiltInParameter, ElementId
        try:
            self.handlerOverrideElems.elems = self.sendChecksResult['{}||{}'.format(self.nodSel.Name, ' '.join(self.nodSel.Text.split(' ')[1:]))]['trueElems']
    ...
    def getTrueElems(self, sender, event):
        ...
        try:
            ...
            if self.doc.Title in self.sendChecksResult[selectedNode]['trueElems']:
                elemIds = [ElementId(int(i)) for i in self.sendChecksResult[selectedNode]['trueElems'][self.doc.Title]]
            ...
            try:
                ...
                for data in dataResult[3:]:
                    ...
                    if status in ['TRUE', 'FALSE']:
                        ...
                        result_[nameCheck] = OrderedDict()
                        result_[nameCheck]['Status'] = status
                        result_[nameCheck]['Category'] = [category]
                        result_[nameCheck]['CategoryRus'] = [categoryRus]
                        result_[nameCheck]['Checks'] = OrderedDict()
                        result_[nameCheck]['Checks']['Check{}'.format(counterChecks)] = OrderedDict()
                        result_[nameCheck]['Checks']['Check{}'.format(counterChecks)][
    ...
    def afterSelectSheet(self, sender, event):
        ...
        try:
            ...
            for data in dataResult[4:]:
                try:
                    articleBefore = data[columnTypeComment-1]
                    article = data[columnTypeComment]
                    if len(article) !=0 and len(articleBefore) != 0 and article not in self.resultArticle.keys():
                    ...
                    try:
                        categoryRus = self.setChecks[nameCheck]['CategoryRus']
                        category = [categoriesJson[i] for i in categoryRus]
                        # category = categoryRus
                    except Exception as e:
                        categoryRus,category = None, None

                    if nameCheck in self.setChecks.keys():
                        ...
                        self.result[nameCheck]['Checks'] = self.setChecks[nameCheck]['Checks']
                        self.resultArticle[nameCheck] = flagArticle
                except Exception as e:
                    print(e)
                    ...
                    for checkKey, checkValue in checks.items():
                        ...
                        for elem in elems:
                            ...
                            if False in centerBoolCheck:
                                ...
                                if elem.Document.Title not in self.resultAllChecks['trueElems']:
                                    self.resultAllChecks['trueElems'][elem.Document.Title] = [elem.Id.ToString()]
                                else:
                                    self.resultAllChecks['trueElems'][elem.Document.Title].append(elem.Id.ToString())
    ...
    def updateTreeViewNodes(self):
        try:
            ...
            for check, boolStatus in self.sendChecksResult.items():
                ...
                article = self.resultArticle[check]
