    ...
    for sheet in sheets:
        ...
        if sheet.title == googleNameSheet:
            ...
            for i, data in enumerate(sheet_data):
                googleIdCheck   = data[5]
                # googleCodeCheck = data[2]
                # googleNameCheck = data[3]
                checkIdResult = googleIdCheck
                columnStatus = 7

                counter += 1
                if counter < 3:
                    continue
                if checkIdResult != '' and checkIdResult in resultChecks.keys():
                    ...
                    itogBoolStatus = resultChecks[checkIdResult]['itogBoolValue']
                    passed_count_elems = int(resultChecks[checkIdResult]['passed_count_currentFile']) + int(resultChecks[checkIdResult]['passed_count_linkFile'])
                    isPassedCheck = resultChecks[checkIdResult]['isPassedCheck']
